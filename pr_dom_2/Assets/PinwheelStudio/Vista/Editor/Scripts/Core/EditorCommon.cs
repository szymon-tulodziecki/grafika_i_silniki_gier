...
namespace Pinwheel.VistaEditor
{
    public static class EditorCommon
    {
        ...
        public static void DrawAffLinks(string text, string url, string utmMedium)
        { 
            DrawAffLinks(text, new string[] { url }, utmMedium);
        }

        public static void DrawAffLinks(string text, string[] urls, string utmMedium)
        {
            if (EditorSettings.Get().generalSettings.enableAffLinks)
            {
                ...
                if (GUI.Button(r, content, AffLinkGUI.LinkButtonStyle))
                {
                    foreach (string s in urls)
                    {
                        ...
                        {
                            string url = NetUtils.ModURL(s, utmMedium);
                            Application.OpenURL(url);
                        }
                    }
                }
